...
public class CubePUi3D extends PApplet implements Cube.Listener {
    ...
    public final void applyConfig(@NotNull Config config) {
        // TODO: config Sounds and poly rhythm
...
//        mPolyRhythmEnabled = config.getValueBool(R.CONFIG_KEY_POLY_RHYTHM, mPolyRhythmEnabled);
        ...
        final float speed = config.getValueFloat(R.CONFIG_KEY_ANIMATION_SPEED, -1f);
        if (speed >= 0 && speed <= 100)
            setMoveSpeedPercent(speed);
    }
...
//        colorMode(ARGB);
    ...
    public void doSolveWorker() {
        ...
        mSolving = true;
        int n = getCube().n();
        U.v(R.SHELL_SOLVER, String.format("Solving %dx%d state: %s", n, n, cubeRepr2D()));
        ...
        try {
            ...
            if (solution.isEmpty()) {
                U.v(R.SHELL_SOLVER, "Already solved!");
            } else {
            ...
            U.e(R.SHELL_SOLVER, "Solver Failed: " + exc.getMessage());
            invalidateSolution();
        }
