 ...
 * Cardan angles from can be done using the following single line of code:
 * </p>
 * 
 * <pre>
...
public class Rotation implements Serializable {
	...
	/**
	 * Inverses a rotation. Build a rotation which inverses the effect of another
	 * rotation. This means that if r(u) = v, then r.inverse()(v) = u. The instance
	 * is not changed.
	 * 
	 * @return a new rotation whose effect is the inverse of the effect of the
	 *         instance
	 */
	 ...
	 * at all times, as in
	 *
	 * <pre>float[] rotations = getAngles(rotations);
	 *rotateX(rotations[0]);
	 *rotateY(rotations[1]);
	 *rotateZ(rotations[2]);
	 *text("Here I am!", 0, 0, 0);</pre>
	 */
	public double[] getAnglesXYZ() {
		try {
            return getAngles(RotationOrder.XYZ);
		} catch (final CardanEulerSingularityException ignored) {
		}

		try {
			final double[] angles = getAngles(RotationOrder.YXZ);
			return new double[] { angles[1], angles[0], angles[2] };
		} catch (final CardanEulerSingularityException ignored) {
		}

		try {
			final double[] angles = getAngles(RotationOrder.ZXY);
			return new double[] { angles[2], angles[0], angles[1] };
		} catch (final CardanEulerSingularityException ignored) {
		}
	...
	/**
	 * wrapper over {@link #getAnglesXYZ()} which returns angles as a float array
	 * */
	public float[] getAnglesXYZ_F() {
		final double[] rots = getAnglesXYZ();

		return new float[] { (float) rots[0], (float) rots[1], (float) rots[2]};
	}
