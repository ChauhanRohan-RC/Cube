...
public class Search {
	 ...
	 */
	public static String solution(String facelets, int maxDepth, long timeOut, boolean useSeparator, @Nullable CancellationProvider canceller) throws CancellationException {
		int s;
		...
		do {
			...
			if (minDistPhase1[n + 1] == 0 && n >= depthPhase1 - 5) {
				...
				if (n == depthPhase1 - 1 && (s = totalDepth(depthPhase1, maxDepth, canceller)) >= 0) {
					if (s == depthPhase1
	...
	static int totalDepth(int depthPhase1, int maxDepth, @Nullable CancellationProvider canceller) {
		int mv = 0, d1 = 0, d2 = 0;
