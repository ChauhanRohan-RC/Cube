...
public class CubeGL implements Drawable, Cube.Listener, MoveGL.Listener {
    ...
    public int finishRunningMoves(boolean cancel) {
        final int size = mRunningMoves.size();
        if (size == 0)
            return 0;
    ...
    public int cancelPendingMoves() {
        final int size = mPendingMoves.size();
        mPendingMoves.clear();
        return size;
    }

    public int cancelAllMoves() {
        return cancelPendingMoves() + finishRunningMoves(true);     // first cancel pending to make sure they are not called after running moves
    }

    public int finishAllMoves(boolean cancel) {
        if (cancel)
            return cancelAllMoves();
        ...
        for (MoveGL m: all) {
            ...
            m.finish(false);  // Since listener is detached, must be handled manually
            commitInternal(m);
        }
