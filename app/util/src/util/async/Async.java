...
//    }
    ...
    public static void sleepCurrentThread(long totalDelayMs, @Nullable CancellationProvider c, long delayPerIterationMs) throws InterruptedException, CancellationException {
        if (totalDelayMs <= 0)
            return;
        ...
        if (c == null) {
            ...
            final long chunkMs = Math.min(totalDelayMs, delayPerIterationMs);
            long sleptMs = 0;
            ...
            do {
                ...
                throwIfCancelled(c);
                sleptMs += chunkMs;
            } while (sleptMs < totalDelayMs);
        }
    }

    public static void sleepCurrentThread(long totalDelayMs, @Nullable CancellationProvider c) throws InterruptedException, CancellationException {
        sleepCurrentThread(totalDelayMs, c, 10 /* 10 ms */);
    }
